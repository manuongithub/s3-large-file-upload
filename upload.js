document.getElementById('uploadButton').addEventListener('click', async () => {
  ...
  async function getSignedUrlWithRetry(partNumber, retries = 0) {
      try {
          ...
          if (retries < maxRetries) {
              ...
              return getSignedUrlWithRetry(partNumber, retries + 1);
          } else {
  ...
  async function uploadChunkWithRetry(signedUrl, chunk, partNumber, retries = 0) {
      try {
          ...
          uploadedChunks++;
          const progress = (uploadedChunks / numberOfChunks) * 100;
          ...
          if (retries < maxRetries) {
              ...
              await new Promise(resolve => setTimeout(resolve, Math.pow(2, retries) * 1000)); // Exponential backoff
              const newSignedUrl = await getSignedUrlWithRetry(partNumber); // Get a new signed URL if needed
              return uploadChunkWithRetry(newSignedUrl, chunk, partNumber, retries + 1);
          } else {
  ...
  try {
      ...
      for (let i = 0; i < numberOfChunks; i++) {
          signedUrlPromises.push(getSignedUrlWithRetry(i + 1));
      }
